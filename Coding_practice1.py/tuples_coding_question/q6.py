...
where m is the length of the second input tuple. Since the two input tuples are of equal length,
the overall time complexity of the function is O(n).
The space complexity is also O(n) because the size of the resulting set will be no larger
than the size of the smaller of the two input tuples.'''

#Therefore, the overall time complexity of the function is O(n)
'''and the '''
#overall space complexity is also O(n),
'''where n is the length of the input tuples.'''
